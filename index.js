...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
//Because const is function scoped, when called in the nested function it reaches out to see if there is any instance of it, within the whole myFunction.
...
function summation(num) {
  let counter = 0;
  //  counter = num.length.reduce((total, added) => total + added,0)
  for (let i=1; i<=num; i++){
    counter+=i
  }
  return counter
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(data){
    const lowerNames = data.map(item => item.animal_name.toLowerCase())
    return lowerNames
  }
  ...
  */
  ...
  function lowPopulationAnimals(data){
    const lowPopElements = data.filter(item => item.population<5)
    return lowPopElements
  }
  ...
  */
  ...
  function USApop(data){
    const totalPopulation = data.reduce((total,num) => total + num.population,0)
